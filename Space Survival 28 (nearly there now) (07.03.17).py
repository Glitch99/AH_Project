...
def bubbleSort(inputData,ranging):
    ...
    while sorted == False:
        outerloop = (len(ranging))
        swaps = 0
        
        for i in range (outerloop - 1):
            comparisons = comparisons + 1
            print(inputData[1][1])
            if inputData[1][i] > inputData[1][i+1]:
                temp = inputData[1][i]
                inputData[1][i] = inputData[1][i+1]
                inputData[1][i+1] = temp
                swaps = swaps + 1
    ...
    if worldState == 3:
        ...
        for event in events:
            if event.type == pygame.KEYDOWN or event.type == pygame.MOUSEBUTTONDOWN: # Change to ENTER pressed
                    ...
                    playerName = "Mah naem Jeff"         ##input("Please Enter Your Name: ")   
                
                    scoreList = open("scoreList.csv", "r")
                    reader = csv.reader(scoreList)
...
## SORT SCORES ##
                    ...
                    finalCSV = bubbleSort(megaList, names)
                    scoreWrite = open("scoreList2.csv","w")
                    for a in range(len(scores)):
                        scoreWrite.write(str(finalCSV[0][a]))
                        print(str(finalCSV[0][a]))
                        scoreWrite.write(str(finalCSV[1][a]))
                        print(str(finalCSV[1][a]))
                    scoreWrite.close
