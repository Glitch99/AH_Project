...
space = pygame.image.load("background\space.png")   
spaceScroll = 0
   
def createSlice(count):
    if count != 4:
        sliced = spaceSlice(0, 0, "space\space" + str(count) + ".png")
        sliced.add(slices)
        createSlice(count + 1)

def hyperSpace(slices,space,spaceScroll):
    
    screen.fill(black)
    
    spaceScroll -= spaceScroll * 1    
    screen.blit(space, [spaceScroll,0])                          

#    slices[0] -= baseMult * 1.6                                 # Increases the speed at which the backgrounds scroll
 #   slices[1] -= baseMult * 1.4
  #  slices[2]  -= baseMult * 1.2
   # slices[3] -= baseMult * 1

#    if slices[0] <= -1000 or slices[0] >= 1000:                                         # Resets the backgrounds should they go past a certain point, creating a
 #       slices[0] = 0                                                                       # psuedo-infinite background
        
  #  if slices[1] <= -1000 or slices[1] >= 1000:
   #     slices[1] = 0  
        
   # if slices[2] <= -1000 or slices[2] >= 1000:
    #    slices[2] = 0  
    #    
    #if slices[3] <= -1000 or slices[3] >= 1000:
     #   slices[3] = 0
...
while isRunning == True:
    ...
    while worldState == 0:
        ...
        hyperSpace(slices,space,spaceScroll) 
